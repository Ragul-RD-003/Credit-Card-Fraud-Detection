import streamlit as st import pandas as pd import numpy as np from PIL import Image, ImageDraw, ImageEnhance from skimage import color from io import BytesIO import base64

========== CONFIGURATION ==========

st.set_page_config(page_title="Pro Color Detection Tool", layout="wide") st.markdown(""" <style> .css-1offfwp { padding: 1rem 2rem; } .zoomed:hover { transform: scale(1.3); transition: 0.3s ease-in-out; z-index: 100; cursor: zoom-in; } .centered { display: flex; justify-content: center; align-items: center; } </style> """, unsafe_allow_html=True)

========== LOAD COLOR DATA ==========

@st.cache_data def load_colors(): df = pd.read_csv("colors.csv") df["LAB"] = df.apply(lambda row: color.rgb2lab(np.array([[row[["R","G","B"]]]], dtype=np.uint8)/255.0)[0][0], axis=1) return df

colors = load_colors()

========== UTILITIES ==========

def rgb_to_lab(rgb): return color.rgb2lab(np.array([[rgb]], dtype=np.uint8)/255.0)[0][0]

def get_closest_colors(R, G, B, top_n=3): input_lab = rgb_to_lab([R, G, B]) distances = colors["LAB"].apply(lambda lab: np.linalg.norm(input_lab - lab)) closest = colors.loc[distances.nsmallest(top_n).index] closest = closest.assign(DeltaE=distances[closest.index]) return closest

def get_hex(rgb): return '#{:02x}{:02x}{:02x}'.format(*rgb)

def pil_to_base64(img): buf = BytesIO() img.save(buf, format="PNG") return base64.b64encode(buf.getvalue()).decode()

def simulate_colorblind(img_np): # Simplified grayscale sim for protanopia return np.dot(img_np[...,:3], [0.299, 0.587, 0.114]).astype(np.uint8)

========== MAIN APP ==========

st.title("Advanced Interactive Color Detection")

uploaded = st.file_uploader("Upload an image", type=["png", "jpg", "jpeg"]) if uploaded: img = Image.open(uploaded).convert("RGB") img_np = np.array(img) h, w = img_np.shape[:2]
