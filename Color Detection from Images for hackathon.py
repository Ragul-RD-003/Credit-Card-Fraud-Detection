...
if uploaded_file is not None:
    ...
    st.write(f"**Image Size:** {width} x {height} (Width x Height)")

    st.write("**Instructions:**")
    st.markdown("""
    - Use the X and Y inputs below to select a pixel position.
    - Top-left corner is (0, 0).
    - Increase X to move right, increase Y to move down.
    - Then click 'Detect Color' to view the result.
    """)

    x = st.number_input("X position", min_value=0, max_value=width-1, value=0)
    y = st.number_input("Y position", min_value=0, max_value=height-1, value=0)
